param(
    ...
    [string]$OutputRoot = "$env:USERPROFILE\Desktop\SpellRise_TestBuild\Client",
...
function Resolve-SteamApi64Source {
    ...
    $projectCandidate = Join-Path $ProjectRoot "Build\SteamRedist\Win64\steam_api64.dll"
    ...
    if (Test-Path $steamworksRoot) {
        ...
        if ($null -ne $latest) {
            $engineCandidate = Join-Path $latest.FullName "Win64\steam_api64.dll"
...
if ($CleanOutput.IsPresent -and (Test-Path $OutputRoot)) {
    Remove-Item -LiteralPath $OutputRoot -Recurse -Force
}

New-Item -ItemType Directory -Path $OutputRoot -Force | Out-Null

Write-Host "[Package] Client Win64 ($Config) incremental..."
& $runUat BuildCookRun `
    -project="$projectPath" `
    -noP4 `
    -utf8output `
...
foreach ($candidate in @(
    (Join-Path $OutputRoot "WindowsClient\steam_appid.txt"),
    (Join-Path $OutputRoot "WindowsClient\SpellRise\Binaries\Win64\steam_appid.txt")
)) {
...
$clientBinDir = Join-Path $OutputRoot "WindowsClient\SpellRise\Binaries\Win64"
if (Test-Path $clientBinDir) {
    Stage-SteamRuntime `
        -TargetBinDir $clientBinDir `
...
Write-Host "[Package] Client concluido com sucesso."
Write-Host "[Package] Client: $OutputRoot"
