param(
    ...
    [string]$OutputRoot = "$env:USERPROFILE\Desktop\SpellRise_TestBuild\Server",
...
Write-Host "[Package] Server Win64 ($Config) incremental..."
& $runUat BuildCookRun `
    -project="$projectPath" `
    -noP4 `
    -utf8output `
...
foreach ($candidate in @(
    (Join-Path $OutputRoot "WindowsServer\steam_appid.txt"),
    (Join-Path $OutputRoot "WindowsServer\SpellRise\Binaries\Win64\steam_appid.txt")
)) {
...
$serverBinDir = Join-Path $OutputRoot "WindowsServer\SpellRise\Binaries\Win64"
if (Test-Path $serverBinDir) {
    Stage-SteamRuntime `
        -TargetBinDir $serverBinDir `
...
Write-Host "[Package] Server concluido com sucesso."
Write-Host "[Package] Server: $OutputRoot"
