...
#include "Widgets/SettingControls/ComboBoxSettingWidget.h"

void UComboBoxSettingWidget::NativeConstruct()
{
	Super::NativeConstruct();

	if (IsValid(ComboBox))
	{
		ComboBox->OnSelectionChanged.AddUniqueDynamic(this, &UComboBoxSettingWidget::ComboBoxSelectionChanged);
	}
}

void UComboBoxSettingWidget::UpdateSelection_Implementation(const FString& Value)
{
...
void UComboBoxSettingWidget::UpdateOptions_Implementation(const TArray<FSettingOption>& InOptions)
{
...
void UComboBoxSettingWidget::ComboBoxSelectionChanged(FString SelectedItem, ESelectInfo::Type SelectionType)
{
	if (!IsUpdatingSettingSelection() && !IsUpdatingSettingOptions())
	// Ignore selection changes while the setting is updating selection or options
	{
