...
{
	return GetChildSettings(UserWidget, Parent).ContainsByPredicate([](UAutoSettingWidget* Setting)
	{
		return Setting->HasUnappliedChange();
	});
}
...
bool USettingContainerUtils::DoesAnyChildSettingHaveUnsavedChange(UUserWidget* UserWidget, UWidget* Parent)
{
	return GetChildSettings(UserWidget, Parent).ContainsByPredicate([](UAutoSettingWidget* Setting)
	{
		return Setting->HasUnsavedChange();
	});
}
